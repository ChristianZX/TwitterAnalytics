...
# TODO: More precise summary of the function (what does it do? where does it get followers from? what does it do with
#  them?)
def get_followers(sql: str) -> None:
    """
    Gets followers from a list retrieved from SQL DB. To retrieve only followers for one user, use API_Followers()
    ...
    if limit == 0:
        ...
        print("Reset Time: " + str(datetime.utcfromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')))
        # TODO: Prints "aborting query", but then continues normally?
    else:
    ...
    for index, element in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        # TODO: More precise Exception (KeyError?)
        except:
...
def download_user_timelines():
    ...
    # List Definition
    # TODO: Why re-define the unpolitical_list variable?
    ...
    # List Download
    # TODO: more elegant and short variation would be to create a dictionary instead of four lists
    #  make the dictionary keys the name of the list (f.i. "unpolitical_list")
    #  and have only one loop:
    ...
    for element in unpolitical_list:
        TwitterAPI.API_tweet_multitool(element, "unpolitical_list", pages=10, method='user_timeline', append=True,
                                       write_to_db=True)
    
    for element in political_list:
        TwitterAPI.API_tweet_multitool(element, "political_list", pages=10, method='user_timeline', append=True,
                                       write_to_db=True)
    
    for element in political_moderate_list:
        TwitterAPI.API_tweet_multitool(element, "political_moderate_list", pages=10, method='user_timeline',
                                       append=True, write_to_db=True)
    
    for element in right_wing_populists_list:
        TwitterAPI.API_tweet_multitool(element, "right_wing_populists_list", pages=10, method='user_timeline',
                                       append=True, write_to_db=True)


# TODO: Add parameters for file paths/names for better parametrization
def pickle_file_load_launcher():
    """
    Loads two files from pickle:
        1) TFIDF converter
        2) Random Forest model for Political / Unpolitical prediction
...
def user_analyse_launcher(iterations: int, sql: str) -> None:
    ...
    # TODO: add paths of pickle files once function has new parameters for file paths
    TFIDF_pol_unpol_conv, Algo_pol_unpol = pickle_file_load_launcher()
    # TODO: add model_name parameter (named of pretrained model or checkpoint path)
...
# TODO: Add missing parameters to docstring, translate docstring to English, add missing type hints
def prediction_launcher(table_name: str, TFIDF_pol_unpol_conv, Algo_pol_unpol, BERT_model, sql: str,
    ...
    if df.shape[1] != 2:
        ...
        # TODO: WHen exiting the program, there's no need to call the garbage collector
        gc.collect()
        sys.exit()
    gc.collect()
    ...
    for index, element in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        for method in methods:
            prediction_result = []
            if method == "pol":
                # TODO: Why reset/rename these variables? They don't seem to be used anywhere else
                algo = Algo_pol_unpol
                tfidf = TFIDF_pol_unpol_conv
                # TODO: Why a separate/duplicate if statement below? The prediction result could be appended here
            
            if method == 'pol':
                prediction_result.append(TFIDF_inference.TFIDF_inference(df_tweets['tweet'], tfidf, algo))
            
            # TODO: Could use "else" instead of another "if", as method can only be pol or LR
            if method == 'LR':
...
def get_friends(sql: str):
    """
    Downloads followers of accounts. Account IDs are given in form of an SQL statement.
    ...
    for index, element in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        # TODO: Profile runtime at some point, the frequent calls to the garbage collector might slow the program down
...
def refresh_score_in_DB(sql: str, get_data_from_DB: bool) -> None:
    ...
    # TODO: German variable names a re bad practice
    ...
    # TODO: SQL is unused, delete/define conditions under which it is used?
...
def get_combined_scores_from_followers(sql: str):
    """
    ...
    # TODO: Fore brevity, multiple empty lists could be instantiated in one line
    ...
    # TODO: These caps should probably be defined in a config file in a future version (and thus become parameters
    #  of this function)
    ...
    for index, element in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        result, rated_accounts, rating_less_accounts, to_few_bert_friends_to_rate_and_LRself_is_invalid, \
        bert_friends_result_is_mediocre, uncategorized_accounts = calculate_combined_score(
        ...
        count_to_few_bert_friends_to_rate_and_LRself_is_invalid.append(
            to_few_bert_friends_to_rate_and_LRself_is_invalid)
    ...
    # TODO: Hide this block in a print function to make this function more concise?
    # TODO: Calculate accounts without rating in a variable before the print statement so 1) you can respect PEP8 and 2)
    #  the print statement is more readable?
    print("\n\n")
    ...
    print(
        f"\nAccounts without rating: "
        f"{count_rating_less_accounts + count_to_few_bert_friends_to_rate_and_LRself_is_invalid + count_bert_friends_result_is_mediocre + count_uncategorized_accounts}")
    ...
    update_sql = 'update n_users set combined_rating = t."0", combined_conf = cast(t."1" as numeric) from temp_table ' \
                 't where n_users.id = t.index'
    db_functions.update_table(update_sql)  # runtime 8 minutes
    db_functions.drop_table("temp_table")


def get_BERT_friends_scores_from_friends(sql: str):
    """
    ...
    """
    df = db_functions.select_from_db(sql)
    # TODO: would be a sensible function parameter
    min_required_bert_friend_opinions = 10
    ...
    # TODO: Is del necessary here? They go out of scope 5 lines later
    del df
...
if __name__ == '__main__':
    # TODO: MOve setup to separate function
    ###setup (not recurring)
    
    # Provide User_Id of followers you want to download in to table n_followers
    # sql = "select u.id from n_users u where cast (u.followers_count as bigint) >= 10000 and u.id not in (select
    # distinct user_id from n_followers) and u.id not in (select distinct user_id from n_friends) and (u.location
    # like '%Deutschland%' or u.location like '%Germany%')"
    ...
    # sql = "select follows_ids as id from (		select fr.follows_ids, count(fr.follows_ids)		from
    # facts_hashtags f, n_friends fr 		where from_staging_table like '%le0711%'		and f.user_id =
    # fr.user_id		group by fr.follows_ids 		having count(fr.follows_ids) >= 500		order by count(
    # fr.follows_ids) desc 		) a	except 	select cast (user_id as text) from n_followers"
    # get_followers(sql)
    
    # Downloads timelines for users configured in function. Used as training material for AI
    ...
    # sql = "SELECT DISTINCT id AS user_id, screen_name AS username FROM n_users u, (SELECT follows_ids,
    # COUNT (follows_ids) FROM n_friends f GROUP BY follows_ids HAVING COUNT (follows_ids) >= 100) a WHERE CAST (
    # a.follows_ids AS bigint) = u.id AND lr IS NULL LIMIT 30"
    # user_analyse_launcher(iterations=250, sql)
    
    ###improve L/R rated user base for better predictions
    # Give LR rating to accounts that FOLLOW many accounts that are in n_followers (Bert inference for users in
    # n_users without self_Bert score, who follow at least 100 Big Users)
    # sql = "SELECT DISTINCT id AS user_id,                 screen_name AS username FROM n_users u,
    # (SELECT follows_ids,          COUNT (follows_ids)    FROM n_followers f   GROUP BY follows_ids    HAVING COUNT
    # (follows_ids) >= 100) a  WHERE CAST (a.follows_ids AS bigint) = u.id AND lr IS NULL limit 200"
    ...
    # Since it affects all users that qualify for a LR rating, it also affects users of any downloaded hashtag you
    # want to analyse
    # sql = "select f.follows_ids, u.screen_name, u.lr as bert_self, f.user_id, u2.lr  as bert_friends from n_users
    # u, n_followers f, n_users u2 where u.id = cast (f.follows_ids as bigint) and u2.id = f.user_id and u2.lr in (
    # 'links','rechts') order by follows_ids"
    # refresh_score_in_DB(sql ,get_data_from_DB = True)
    
    # TODO: Move hashtag download to separate function
    ...
    # sql = "select f.user_id from facts_hashtags f where from_staging_table like '%le0711%' except select distinct
    # f.user_id from facts_hashtags f, n_friends fr where from_staging_table like '%le0711%' and f.user_id = fr.user_id"
    # get_friends(sql)
    
    # Give Bert LR-self rating to all new users in hashtag
    # The LR-self rating is based on the users tweets
    # hashtag = 'le0711'
    # sql = f"SELECT DISTINCT u.id AS user_id, screen_name AS username FROM n_users u, (SELECT follows_ids,
    # COUNT (follows_ids) FROM n_followers f GROUP BY follows_ids HAVING COUNT (follows_ids) >= 100) a ,
    # facts_hashtags fh WHERE CAST (a.follows_ids AS bigint) = u.id AND lr IS NULL AND fh.user_id = u.id AND
    # fh.from_staging_table like '%{hashtag}%' LIMIT 200"
    # user_analyse_launcher(iterations=1, sql=sql)
    # Overwrite / Refresh LR-self rating for all users of a hashtag
    # sql = f"select distinct user_id from facts_hashtags f, n_users u where f.hashtags = '{hashtag}' and f.user_id =
    # u.id and u.lr is null"
    ...
    hashtag = 'le0711'
    sql = f"SELECT distinct f.follows_ids,       u.screen_name,       u.lr AS bert_self,       f.user_id,       " \
          f"u2.lr AS bert_friends FROM n_users u,     n_followers f,     n_users u2,	 facts_hashtags fh WHERE u.id " \
          f"" \
          f"" \
          f"" \
          f"= CAST (f.follows_ids AS bigint)  AND u2.id = f.user_id  AND u2.lr in ('links', 'rechts')  AND fh.user_id " \
          f"" \
          f"" \
          f"" \
          f"= u.id   AND fh.from_staging_table like '%{hashtag}%' ORDER BY follows_ids"
    refresh_score_in_DB(sql, get_data_from_DB=True)
    
    # Calculates combined score from users self-LR score and users bert_friend score
    # sql = f"SELECT id,		   screen_name,		   lr,		   lr_conf,		   result_bert_friends,
    # bert_friends_conf,		   bf_left_number,		   bf_right_number FROM n_users	WHERE (lr IS NOT NULL
    # OR result_bert_friends IS NOT NULL)		   and combined_rating is null"
    # get_combined_scores_from_followers(sql)
    
    # Counts how many left or right friends a user has. Unlike seemingly similar functions (who download and analyse
    # tweets),
    # this one gets user profiles from DB as DF.
    # For users that need special attention because they did not get a combined score due to bad BERT_friend score.
    # conf_level = 0.75
    # hashtag = 'le0711'
    # sql = f"select distinct u.id, u2.id as friend_id, u2.lr, u2.lr_conf, u2.result_bert_friends,
    # u2.bert_friends_conf, u2.bf_left_number, u2.bf_right_number,u2.combined_rating, u2.combined_conf from
    # facts_hashtags f, n_users u, n_friends fr, n_users u2 where from_staging_table like '%{hashtag}%' and f.user_id
    # = fr.user_id and fr.user_id = u.id and cast (fr.follows_ids as numeric) = u2.id and u.combined_rating is null
    # and u2.result_bert_friends in ('links','rechts') and u2.bert_friends_conf >= {conf_level} order by u.id"
